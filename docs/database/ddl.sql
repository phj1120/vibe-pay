...
COMMENT ON COLUMN PAY_BASE.PG_TYPE_CODE IS 'PG 코드(PAY005 / 001 이니시스 / 002 나이스 / 999 테스트PG)';
...
COMMENT ON COLUMN PAY_INTERFACE_LOG.PAY_LOG_CODE IS '로그유형코드 (PAY_004: 001-결제/002-승인/003-망취소/004-취소)';
...
COMMENT ON COLUMN POINT_HISTORY.UPPER_POINT_HISTORY_NO IS '원포인트기록번호';
COMMENT ON COLUMN POINT_HISTORY.REMAIN_POINT IS '잔여유효포인트';
...
(
    ...
    GOODS_MAIN_IMAGE_URL VARCHAR(200) NOT NULL,               -- 상품대표이미지주소
    REGIST_ID            VARCHAR(15)  NOT NULL,               -- 등록자
    REGIST_DATE_TIME     TIMESTAMP    NOT NULL DEFAULT NOW(), -- 등록일시
    MODIFY_ID            VARCHAR(15)  NOT NULL,               -- 수정자
    MODIFY_DATE_TIME     TIMESTAMP    NOT NULL DEFAULT NOW(), -- 수정일시
...
COMMENT ON COLUMN GOODS_BASE.GOODS_MAIN_IMAGE_URL IS '상품대표이미지주소';
...
(
    GOODS_NO          VARCHAR(15) NOT NULL,               -- 상품번호 (PK)
    ITEM_NO           VARCHAR(3)  NOT NULL,               -- 단품번호 (PK)
    ITEM_NAME         VARCHAR(50) NOT NULL,               -- 단품명
...
COMMENT ON COLUMN GOODS_ITEM.ITEM_NAME IS '단품명';
