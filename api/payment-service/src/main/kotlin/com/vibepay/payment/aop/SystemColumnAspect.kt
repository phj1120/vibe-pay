package com.vibepay.payment.aop
...
class SystemColumnAspect {
    ...
    @Before("execution(* com.vibepay.payment.repository..*.save*(..)) && args(entity,..)")
    fun setSystemColumns(joinPoint: JoinPoint, entity: Any) {
        if (entity !is SystemEntity) return
        val userId = resolveUserId()
        if (entity.registId.isEmpty()) entity.registId = userId
    ...
    private fun resolveUserId(): String {
        return try {
            ...
            log.warn("Could not resolve user id: {}", e.message)
            "system"
        }
    }
}
