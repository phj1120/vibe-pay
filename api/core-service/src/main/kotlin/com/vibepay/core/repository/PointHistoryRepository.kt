...
interface PointHistoryRepository : JpaRepository<PointHistory, String> {
    ...
    @Query(value = "SELECT COALESCE(SUM(REMAIN_POINT), 0) FROM POINT_HISTORY WHERE MEMBER_NO = :memberNo AND POINT_TRANSACTION_CODE = '001' AND REMAIN_POINT > 0 AND END_DATE_TIME > NOW()", nativeQuery = true)
    fun selectPointBalance(memberNo: String): Long

    @Query(
        value = """
        SELECT POINT_HISTORY_NO AS pointHistoryNo, AMOUNT AS amount, POINT_TRANSACTION_CODE AS pointTransactionCode,
               POINT_TRANSACTION_REASON_CODE AS pointTransactionReasonCode, POINT_TRANSACTION_REASON_NO AS pointTransactionReasonNo,
               START_DATE_TIME AS startDateTime, END_DATE_TIME AS endDateTime, REMAIN_POINT AS remainPoint, REGIST_DATE_TIME AS createdDate
          FROM POINT_HISTORY WHERE MEMBER_NO = :memberNo ORDER BY REGIST_DATE_TIME DESC LIMIT :size OFFSET :offset
        """,
        nativeQuery = true
    )
    fun selectPointHistoryList(memberNo: String, size: Int, offset: Long): List<PointHistoryProjection>

    @Query(value = "SELECT COUNT(*) FROM POINT_HISTORY WHERE MEMBER_NO = :memberNo", nativeQuery = true)
    ...
    @Query(
        ...
        SELECT POINT_HISTORY_NO, MEMBER_NO, AMOUNT, POINT_TRANSACTION_CODE,
          ...
          FROM POINT_HISTORY
         WHERE MEMBER_NO = :memberNo AND POINT_TRANSACTION_CODE = '001'
           AND REMAIN_POINT > 0 AND END_DATE_TIME > NOW()
    ...
    @Modifying
    @Query(value = "UPDATE POINT_HISTORY SET REMAIN_POINT = :remainPoint, MODIFY_ID = :modifyId, MODIFY_DATE_TIME = NOW() WHERE POINT_HISTORY_NO = :pointHistoryNo", nativeQuery = true)
